...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['checkout_dipilih'])) {
  if (!isset($_POST['pilih'])) {
    echo "<script>alert('Pilih dulu produk yang ingin di-checkout!'); window.location='produk_keranjang.php';</script>";
    exit;
  }
  ...
  foreach ($id_keranjang as $idk) {
    $data = $koneksi->query("SELECT k.id_keranjang, k.id_produk, p.nama_produk, p.harga, p.photo 
                              FROM keranjang k 
                              JOIN produk p ON k.id_produk = p.id_produk 
                              WHERE k.id_keranjang = $idk")->fetch_assoc();
...
<body>
  <div class="checkout-card">
    ...
    <form method="post" action="admin/transaksi_add.php">
      ...
      <div class="total">Total: <span class="text-primary fw-bold">Rp<?= number_format($total_semua, 0, ',', '.') ?></span></div>
